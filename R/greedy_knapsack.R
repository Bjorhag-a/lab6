...
#' @description This function solves the knapsack problem using the greedy
#'   algorithm. The algorithm selects the items based on the value per weight.
#'
#'
#' @param x A data frame with two variables, the value of the item and the
#'   weight of the item.
#' @param W A numeric number that sets the total weight capacity of the knapsack
#'   and must be greater than 0
#' @param fast logical, Set to TRUE and a C++ implementation calculates the
#'   algorithm. Default is FALSE.
#'
#' @returns A list of the selected items total value and a vector of the
#'   selected items observation number
#' @export
#' 
...
greedy_knapsack <-function(x, W, fast=FALSE){
  ...
  if (fast==FALSE) {
    ...
    #For loop adds the val per w till the weight reach the limit set.
    for (i in 1:n) {
    ...
    cppFunction('
      List greedyCpp(NumericVector v, NumericVector w, int W){
        int n = v.size();
